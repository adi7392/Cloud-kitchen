  ...
  return (
    <div className="min-h-screen bg-mm-black">
      ...
      <div className="px-4 py-5 space-y-4 max-w-lg mx-auto pb-16">
        ...
        {/* Borzo Courier Info */}
        {order?.borzoOrderId && (order.borzoCourierName || order.borzoCourierPhone) && (
          <div className="flex items-start gap-3 rounded-2xl p-4 bg-orange-50 border border-orange-100">
            <Truck size={15} className="mt-0.5 shrink-0 text-orange-500" />
            <div className="min-w-0">
              <p className="font-body text-[10px] uppercase tracking-wider mb-1 text-orange-600">Your Delivery Partner</p>
              {order.borzoCourierName && (
                <p className="font-body text-sm font-700 text-orange-900">{order.borzoCourierName}</p>
              )}
              {order.borzoCourierPhone && (
                <a
                  href={`tel:${order.borzoCourierPhone}`}
                  className="font-body text-xs text-orange-700 hover:underline flex items-center gap-1 mt-0.5"
                >
                  📞 {order.borzoCourierPhone}
                </a>
              )}
